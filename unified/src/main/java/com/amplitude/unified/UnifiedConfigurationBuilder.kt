 ...
 *     engagement { options = AmplitudeInitOptions() }
 * }
 * ```
 */
public open class UnifiedConfigurationBuilder(
    ...
    /** Guides and Surveys-specific configuration. */
    public val engagement: EngagementConfigurationBuilder = EngagementConfigurationBuilder()
    ...
    /** Applies [configure] to Guides and Surveys-specific options. */
    public fun engagement(configure: EngagementConfigurationBuilder.() -> Unit): UnifiedConfigurationBuilder =
        apply { engagement.configure() }
    ...
    internal fun buildSnapshot(): UnifiedConfiguration =
        UnifiedConfiguration(
            ...
            engagement = engagement.buildSnapshot(),
        )
}
...
/** Guides and Surveys options owned by the unified wrapper. */
public open class EngagementConfigurationBuilder {
    /** Whether Guides and Surveys is installed. */
    public var enabled: Boolean = true

    /** Product-specific Guides and Surveys options. The Analytics host supplies server zone. */
    public var options: AmplitudeInitOptions = AmplitudeInitOptions()

    internal fun buildSnapshot(): EngagementConfiguration =
...
internal data class UnifiedConfiguration(
    ...
    val engagement: EngagementConfiguration,
)
...
internal data class EngagementConfiguration(
    val enabled: Boolean,
    val options: AmplitudeInitOptions,
)
