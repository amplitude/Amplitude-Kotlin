...
class FrustrationInteractionsDetectorTest {
    ...
    fun `rage click - tracks correct event properties`() {
        ...
        val clickTimestamps = clicks.map { it["Time"] as String }
    ...
    fun `rage click - formats timestamps as UTC`() {
        ...
        try {
            ...
            assertEquals(
                ...
                detector.formatIso8601Timestamp(1_788_845_016_809L),
            )
            assertEquals(
                "2026-09-08T05:23:36.809Z",
    ...
    fun `dead click detection should work after start()`() {
        ...
        val capturedProperties = slot<Map<String, Any?>>()
        verify { mockAmplitude.track(DEAD_CLICK, capture(capturedProperties)) }
        ...
        assertAll(
            { assertTrue(timestampPattern.matches(beginTime)) },
            { assertTrue(timestampPattern.matches(endTime)) },
            { assertEquals(100, capturedProperties.captured[COORDINATE_X]) },
            { assertEquals(100, capturedProperties.captured[COORDINATE_Y]) },
            { assertEquals(3000L, capturedProperties.captured[DURATION]) },
            { assertEquals(1, capturedProperties.captured[CLICK_COUNT]) },
            {
                assertEquals(
                    3000L,
                    Instant.parse(endTime).toEpochMilli() - Instant.parse(beginTime).toEpochMilli(),
